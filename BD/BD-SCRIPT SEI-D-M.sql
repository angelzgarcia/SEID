...
CREATE TABLE IF NOT EXISTS `seid`.`qr_codes` (
  ...
  `id_credencial_qr_code` INT UNSIGNED NOT NULL,
  `file_path` VARCHAR(60) NOT NULL,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id_qr_code`),
  UNIQUE INDEX `id_credencial_qr_codes_UNIQUE` (`id_credencial_qr_code` ASC) VISIBLE,
  CONSTRAINT `id_credencial_qr_codes`
    FOREIGN KEY (`id_credencial_qr_code`)
...
CREATE TABLE IF NOT EXISTS `seid`.`productos` (
  ...
  `codigo_barras_producto` INT NULL,
  `nombre_producto` VARCHAR(250) NOT NULL,
  `slug_producto` VARCHAR(55) NOT NULL,
  `tipo_venta` ENUM('unidad', 'granel', 'paquete') NOT NULL,
  `stock_producto` INT UNSIGNED NOT NULL,
  `precio_costo` FLOAT UNSIGNED NOT NULL,
  `precio_venta` FLOAT UNSIGNED NOT NULL,
  `precio_mayoreo` FLOAT UNSIGNED NULL,
...
CREATE TABLE IF NOT EXISTS `seid`.`ventas` (
  ...
  `total_venta` FLOAT UNSIGNED NOT NULL,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id_venta`),
  INDEX `id_sucursal_idx` (`id_sucursal` ASC) VISIBLE,
  INDEX `id_credencial_idx` (`id_credencial` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `seid`.`ventas_detalles` (
  ...
  `id_venta` INT UNSIGNED NOT NULL,
  `id_producto_sucursal` INT UNSIGNED NOT NULL,
  `cantidad_producto` INT UNSIGNED NOT NULL,
  `subtotal` FLOAT UNSIGNED NOT NULL,
  PRIMARY KEY (`id_venta_detalle`),
  INDEX `id_venta_idx` (`id_venta` ASC) VISIBLE,
  INDEX `id_producto_sucursal_idx` (`id_producto_sucursal` ASC) VISIBLE,
  CONSTRAINT `id_venta`
    FOREIGN KEY (`id_venta`)
    REFERENCES `seid`.`ventas` (`id_venta`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `id_producto_sucursal`
    FOREIGN KEY (`id_producto_sucursal`)
...
CREATE TABLE IF NOT EXISTS `seid`.`egresos` (
  ...
  `monto_egreso` FLOAT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `seid`.`pedidos_sucursales_detalles` (
  ...
  `id_pedido_sucursal` INT UNSIGNED NOT NULL,
  `id_producto_sucursal` INT UNSIGNED NOT NULL,
  `cantidad_producto` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id_pedido_sucursal_detalle`),
  INDEX `id_pedido_sucursal_idx` (`id_pedido_sucursal` ASC) VISIBLE,
  INDEX `id_producto_sucursal_idx` (`id_producto_sucursal` ASC) VISIBLE,
  CONSTRAINT `id_pedido_sucursal`
    FOREIGN KEY (`id_pedido_sucursal`)
