...
$sql = "
    SELECT c.*, s.nombre_sucursal, s.id_sucursal
    FROM credenciales AS c
    INNER JOIN sucursales AS s ON s.id_sucursal = c.id_sucursal_fk_credencial
";

if ($busqueda) {
    $monditions[] = "(c.nombres_credencial LIKE ? OR c.apellidos_credencial LIKE ? OR c.correo_inicio LIKE ? OR c.curp_credencial LIKE ? OR c.telefono_credencial LIKE ?)";
...
$orden = match ($orden) {
    'recientes' => 'id_credencial DESC',
    'antiguas' => 'id_credencial ASC',
    'az' => 'nombre_credencial ASC',
    'za' => 'nombre_credencial DESC',
    default => 'id_credencial DESC'
};

$sql .= " ORDER BY $orden LIMIT 14";

$credenciales = simpleQuery($sql, $params, $types, true) ?: [];

if ($busqueda && empty($credenciales)): ?>
...
<?php elseif (!$busqueda && empty($credenciales)): ?>
    ...
    <div class="registers-empty">
        ...
        <p>AÚN NO HAY CREDENCIALES REGISTRADAS.</p>
    </div>
    ...
    foreach ($credenciales as $credencial): ?>
        ...
        <div class="register-frame category-frame">
            ...
            <div
                ...
                data-id="<?= encryptValue($credencial['id_credencial'], SECRETKEY) ?>"
            >
                <div class="register-details">
                    <div class="header-register user-header-info">
                        <p>
                            <span><?= ucfirst($credencial['nombres_credencial']) ?></span>
                        </p>

                        <p>
                            <span><?= ucfirst($credencial['apellidos_credencial']) ?></span>
                        </p>
                    </div>

                    <div class="body-register category-body-register credential-body-register">
                        <img src="<?= HTTP_URL ?>imgs_denedig/user_4.png" alt="user image" loading="lazy">

                        <div class="quantities">
                            <p>
                                Teléfono:
                                <span><?= ucfirst($credencial['telefono_credencial']) ?? '' ?></span>
                            </p>

                            <p>
                                Correo:
                                <span><?= $credencial['correo_inicio'] ?? '' ?></span>
                            </p>

                            <p>
                                Sucursal:
                                <span><?= $credencial['nombre_sucursal'] ?? '' ?></span>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <?php $c = encryptValue($credencial['id_credencial'], SECRETKEY) ?>
            <?php $status = (int)$credencial['status_credencial'] ?>
