...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // <-- SE ORDENAN LAS credenciales -->
    document.getElementById('order-by-credentials').addEventListener('change', busqueda);
});
...
function buscar() {
    ...
    const orderBy = document.getElementById('order-by-credentials').value;
    ...
    $.ajax({
        data: params,
        type: 'POST',
        url: '../../functions/buscar_credenciales.php',
        success: data => {
            document.getElementById('credentials-container').innerHTML = data;
        ...
        error: (xhr, status, error) => {
            document.getElementById('credentials-container').innerHTML = `
                <div class="registers-empty">
                    ...
                    <p>LO SENTIMOS. NO SE PUDIERON CARGAR CORRECTAMENTE LAS CREDENCIALES.</p>
                </div>
            `;
...
$(document).on('click', '.status-btn ', function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '../../functions/crud_credencial.php',
                method: 'POST',
                data: { c: credentialId, accion: 'status' },
                dataType: 'json',
                ...
                error: (xhr, status, error) => {
                    ...
                    Toast.fire({
                        icon: "error",
                        title: "¡No se encontró la credencial!"
                    });
                }
            });
...
$(document).on('click', '.open-register-details-modal ', function() {
    ...
    $.ajax({
        url: '../../functions/crud_credencial.php',
        method: 'GET',
        data: { s: credentialId, accion: 'detalles' },
