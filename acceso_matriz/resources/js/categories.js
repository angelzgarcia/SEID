...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('order-by-categories').addEventListener('change', busqueda);
});
...
function buscar() {
    ...
    const orderBy = document.getElementById('order-by-categories').value;
    ...
    $.ajax({
        data: params,
        type: 'POST',
        url: '../../../functions/buscar_categorias.php',
        success: data => {
        ...
        error: (xhr, status, error) => {
            document.getElementById('categories-container').innerHTML = `
                <div class="registers-empty">
                    ...
                    <p>LO SENTIMOS. NO SE PUDIERON CARGAR CORRECTAMENTE LAS CATEGORIAS.</p>
                </div>
            `;
...
$(document).on('click', '.open-register-details-modal ', function() {
    ...
    $.ajax({
        url: '../../../functions/crud_categoria.php',
        method: 'GET',
        data: { c: categoryId, accion: 'detalles' },
        success: function(data) {
            if (data.status) {
                ...
                $.each(data, function(key, value) {
                    ...
                    if (key === 'imagen_categoria') {
                        $('.register-fact-image img').attr('src', value);
                    } else if (key === 'status_categoria') {
        ...
        error: (xhr, status, error) => {
            ...
            Toast.fire({
                icon: "error",
                title: "¡No se encontró la categoria!"
            });
        }
    });
});
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '../../../functions/crud_categoria.php',
                method: 'POST',
                data: { c: categoryId, accion: 'status' },
                dataType: 'json',
